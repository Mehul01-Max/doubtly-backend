...
solution.post("/add/:questionId", userMiddleware, async (req, res) => {
    try {
        ...
        const newDoubt = new SolutionDB({doubtID: questionId, userID: req.userId, solution, status: "pending", addDate: new Date()});
        newDoubt.save();
        res.json({
...
solution.put("/modify/:solutionId", userMiddleware, async (req, res) => {
    try {
        ...
        if (req.userId != Solution.userID) {
            return res.status(400).json({
                message: "You are not allowed to modify this solution as you are not the author"
            })
        }
        if (!solution) {
            return res.status(400).json({
        ...
        await SolutionDB.findByIdAndUpdate(solutionId, {solution, modifiedDate: new Date()}, {runValidators: true})
        res.json({
...
solution.delete("/delete/:solutionId", userMiddleware, async (req, res) => {
    try {
        ...
        if (req.userId != Solution.userID) {
            return res.status(400).json({
                message: "You are not allowed to delete this solution as you are not the author"
            })
        }
