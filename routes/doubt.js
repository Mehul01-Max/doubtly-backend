...
doubt.post("/add", userMiddleware, async (req, res) => {
    try {
        ...
        const newDoubt = new DoubtDB({userID: req.userId, heading, description, type, status: false, addDate: new Date()});
        newDoubt.save();
        res.json({
...
doubt.put("/modify/:doubtid", userMiddleware, async (req, res) => {
    try {
        ...
        if (req.userId != Doubt.userID) {
            return res.status(400).json({
                message: "You are not allowed to modify this question as you are not the author"
            })
        }
        if (!heading || !description || !type) {
            return res.status(400).json({
        ...
        await DoubtDB.findByIdAndUpdate(doubtId, {heading, description, type, modifiedDate: new Date()}, {runValidators: true})
        res.json({
...
doubt.delete("/delete/:doubtId", userMiddleware, async (req, res) => {
    try {
        ...
        if (req.userId != Doubt.userID) {
            return res.status(400).json({
                message: "You are not allowed to delete this question as you are not the author"
            })
        }
